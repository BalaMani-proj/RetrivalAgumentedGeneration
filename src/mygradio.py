...
def generate_with_rag(query):
    ...
    # Constructing a clear and direct prompt for the model
    prompt = (
        f"Context: {context}\n"
        f"Question: {query}\n"
        "Answer:"
    ...
    # Generate answer deterministically while allowing minor variation
    output_tokens = model_qa.generate(
        **inputs, 
        max_new_tokens=500,
        do_sample=True,  # Disable sampling for more controlled output
        temperature=0.6,  # Lower temperature to reduce randomness
        top_p=0.9  # Ensure relevant tokens are prioritized
    )
    
    # Extract only the generated answer
    answer = tokenizer_qa.decode(output_tokens[0], skip_special_tokens=True)
    #answer = answer.split("Answer:")[-1].strip()  # Remove extra text before "Answer:"
...
with gr.Blocks() as demo:
    with gr.Row():
        with gr.Column(scale=1, min_width=600):
            ...
            output_text = gr.Textbox(label="Response", interactive=False, lines=5)
            
            query_input.submit(generate_with_rag, inputs=query_input, outputs=output_text)

# Add this line to launch the Gradio UI
demo.launch()